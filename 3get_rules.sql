...
select a.offer,a.uc1_priority, a.active_flg_kd, a.active_flg_fb, a.active_flg_pt, b.dvc_techname, a.min_CTR, a.min_landings, a.min_submits, a.min_S2L, a.max_CPS,  concat_ws(' and ', concat('CTR>=', a.min_ctr), concat('submits>=', a.min_submits), concat('S2L>=', a.min_S2L),concat('CPS<=', a.max_CPS) ) as feedback_cond from (select  * from sgmt_rules.offer_control where cast(start_date as string) <= concat_ws('-',cast(${var:year} as string),cast(${var:month} as string),cast(${var:day} as string)) and concat_ws('-',cast(${var:year} as string),cast(${var:month} as string),cast(${var:day} as string))  < cast(end_date as string) and (active_flg_fb is not null or active_flg_kd is not null or active_flg_pt is not null) ) a join apollo_util.techname_prop_map b on a.target_rlp_min <=cast(b.release_price as double)*40 and cast(b.release_price as double)*40<=a.target_rlp_max group by offer, uc1_priority, active_flg_kd, active_flg_fb, active_flg_pt, dvc_techname, min_CTR, min_landings, min_submits, min_S2L, max_CPS);
...
insert into table sgmt_rules.dynamic_rules_daily partition(year=${var:year}, month=${var:month}, day=${var:day}) 
select * from 
(select 'kd' as source, offer, uc1_priority, concat('userAgent(".*(?i)(',group_concat(trim(dvc_techname),'|'),').*")') as rule from sgmt_rules.offer_dvc_map_kd where dvc_techname not in ('asus','huawei','zte=blade','vivo') group by source, offer, uc1_priority union all
select 'kd_app' as source, offer, uc1_priority, concat('userAgent(".*(?i)(',group_concat(trim(dvc_techname),'|'),').*")') as rule from sgmt_rules.offer_dvc_map_kd_app where dvc_techname not in ('asus','huawei','zte=blade','vivo') group by source, offer, uc1_priority union all
select 'fb' as source, offer, uc1_priority, concat('userAgent(".*(?i)(',group_concat(trim(dvc_techname),'|'),').*")') as rule from sgmt_rules.offer_dvc_map_fb where dvc_techname not in ('asus','huawei','zte=blade','vivo')  group by source, offer, uc1_priority union all
select 'pt' as source, offer, uc1_priority, concat('userAgent(".*(?i)(',group_concat(trim(dvc_techname),'|'),').*")') as rule from sgmt_rules.offer_dvc_map_pt where dvc_techname not in ('asus','huawei','zte=blade','vivo')  group by source, offer, uc1_priority) A order by source asc, uc1_priority asc ;


/*
impala-shell -i impala.prd.sg1.tapad.com:21000 -B -o /local/home/rata.suwantong/rules_170102.csv --output_delimiter=',' -q "select * from sgmt_rules.dynamic_rules order by source asc,uc1_priority asc"
*/